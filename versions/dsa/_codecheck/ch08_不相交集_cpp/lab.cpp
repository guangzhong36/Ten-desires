...
struct NaiveUF {
    ...
    void unite(int a, int b) {
        a = find(a); b = find(b);
        if (a != b) parent_[a] = b;         // 固定挂到 b 下,容易连成长链
    }
};
...
Maze genMaze(int w, int h, int seed = 2024) {
    ...
    Maze m{w, h,
           ...
           vector<vector<bool>>(h, vector<bool>(w, true)),
           {}};
    vector<pair<int, int>> walls;           // 每面墙用两端格子的编号表示
...
void bench(int n) {
    ...
    auto t1 = Clock::now();
    for (int x = 0; x < n; ++x) s += naive.find(x); // 链尾 find 一步 O(n)
    auto t2 = Clock::now();
    UnionFind opt(n);
    ...
    auto t4 = Clock::now();
    cout << "任务4: " << n << " 次 union(连成链)+ " << n << " 次 find(耗时对比):\n";
...
int main() {
    ...
    cout << "\n任务3: 拆墙过程中连通分量数量的变化(共 " << total << " 面候选墙):\n";
    cout << "  初始: " << w * h << " 个连通分量\n";
