...
// 编译: g++ -std=c++17 -O2 lab7.cpp -o lab7   运行: ./lab7
#include <algorithm>
#include <chrono>
#include <iomanip>
#include <iostream>
...
bool g_count = false;           // 任务 5 计数开关,计时期间保持 false
...
vector<int> makeData(int n, Kind k, mt19937& rng) {
    ...
    if (k == RANDOM) {
        ...
        for (int i = 0; i < 20; ++i) {  // 只做 20 次随机交换:逆序对 O(n)
            int x = uniform_int_distribution<int>(0, n - 1)(rng);
            int y = uniform_int_distribution<int>(0, n - 1)(rng);
            swap(a[x], a[y]);
        }
    } else {
        for (int i = 0; i < n; ++i) a[i] = n - i;   // 完全逆序
    }
    return a;
}
// 计时:同数据重复 3 次取最小值;每次用副本;结果读走防死代码消除
...
int main() {
    ...
    bool ok = true;
    vector<int> ref, b;              // 参考答案:ref 为标准库结果
    auto verify = [&](vector<int> x, void (*f)(vector<int>&)) {
        f(x);
        ok &= (x == ref);
    };
    for (int k = 0; k < 3; ++k) {
        vector<int> a = makeData(500, Kind(k), rng);
        ref = a;
        sort(ref.begin(), ref.end());
        verify(a, insertionSortAll);
        verify(a, mergeSort);
        verify(a, quickSort);
    }
    vector<int> dup(500);
    uniform_int_distribution<int> d20(0, 20);
    for (int& x : dup) x = d20(rng);
    ref = dup;
    sort(ref.begin(), ref.end());
    verify(dup, insertionSortAll);
    verify(dup, mergeSort);
    verify(dup, quickSort);
    cout << "任务 3:三种算法与 std::sort 输出一致: " << (ok ? "是" : "否") << "\n";
    // 任务 2:耗时表(插入排序 n>8000 按 ×4 规律标注"(推算)")
    cout << "任务 2:耗时表(单位 ms;插入排序 n>8000 为(推算))\n";
    for (int k = 0; k < 3; ++k) {
        cout << "\n[" << kindName[k] << "]\n";
        cout << left << setw(8) << "n" << setw(11) << "插入"
             << setw(11) << "归并" << "快排\n";
        for (size_t s = 0; s < sizes.size(); ++s) {
            int n = sizes[s];
            vector<int> a = makeData(n, Kind(k), rng);
            cout << setw(8) << n;
            if (n <= 8000) {
    ...
    auto ratios = [&](const char* name, double t[3][7]) {
        for (int k = 0; k < 3; ++k) {
            cout << name << "/" << kindName[k] << ": ";
            for (size_t s = 1; s < sizes.size(); ++s)
                if (t[k][s] > 0 && t[k][s - 1] > 0)
                    cout << sizes[s - 1] << "->" << sizes[s] << " x"
                         << setprecision(2) << (t[k][s] / t[k][s - 1]) << "  ";
            cout << "\n";
        }
    };
    ratios("插入", ins);
    ratios("归并", mrg);
    ratios("快排", qck);
    // 任务 5(加分):比较/移动次数统计(n=1000 随机)
    g_count = true;
    vector<int> a = makeData(1000, RANDOM, rng);
    g_cmp = 0; g_move = 0;
    b = a; insertionSort(b, 0, int(b.size()) - 1);
    ...
    cout << "\n结论:实测比值与理论吻合——插入排序 n 翻倍时间约 x4,"
            "归并/快排约 x2;逆序数据上插入排序退化最明显,"
